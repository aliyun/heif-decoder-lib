...
heif_filetype_result heif_check_filetype(const uint8_t* data, int len)
{
  ...
  if (len >= 12) {
    ...
    else if (brand == heif_brand2_hevc) {
      return heif_filetype_yes_supported;
    }
    else if (brand == heif_brand2_hevx) {
      return heif_filetype_yes_supported;
    }
    else if (brand == heif_brand2_msf1) {
      return heif_filetype_maybe;
    }
    else {
...
void heif_context_set_threads(struct heif_context* ctx, const struct heif_image_handle* in_handle, int nthreads)
{
  ...
  int grid_flag = ctx->context->read_grid_params(id);

  if(grid_flag)
  {
    ctx->context->set_max_decoding_threads(nthreads);
    ctx->context->set_max_decoder_threads(0);
  }
  else
  {
    ctx->context->set_max_decoding_threads(0);
    ctx->context->set_max_decoder_threads(nthreads);
  }
}

heif_error heif_context_get_heif_params(struct heif_context* ctx, libheif_parameters* params)
{
  Error err = ctx->context->read_params(params);
...
struct heif_error heif_context_get_image_info(struct heif_context* ctx, int* width, int* height)
{
...
void fill_default_decoding_options(heif_decoding_options& options)
{
  ...
  options.ext_dst_enable = false ;
  options.ext_dst = NULL;
  options.ext_dst_len = 0 ;
  options.ext_dst_stride = 0;
...
static void copy_options(heif_decoding_options& options, const heif_decoding_options& input_options)
{
  ...
  if(input_options.ext_dst_enable) {
    options.ext_dst_enable = true;
    options.ext_dst        = input_options.ext_dst ;
    options.ext_dst_len    = input_options.ext_dst_len ;
    options.ext_dst_stride = input_options.ext_dst_stride;
  }
}
...
void heif_decoding_options_add_external_dest(struct heif_decoding_options* options, void* dst, uint32_t len, uint32_t stride)
{
  options->ext_dst_enable = true;
  options->ext_dst        = dst ;
  options->ext_dst_len    = len ;
  options->ext_dst_stride = stride;
}
                                    ...
                                    const struct heif_decoding_options* input_options)
{
  ...
  bool has_alpha = in_handle->image->get_alpha_channel() != nullptr ;

  Error err = in_handle->context->decode_image_user(id, img,
                                                    colorspace,
                                                    chroma,
                                                    heif_image_handle_get_width(in_handle),
                                                    heif_image_handle_get_height(in_handle),
                                                    has_alpha,
                                                    dec_options);
                                     ...
                                     void* userdata)
{
  ...
  if(ctx->context->get_movie_flag())
  {
    ctx->context->patch_stco_data(swriter);
  }
...
void heif_set_movie_flag(struct heif_context* ctx, bool movie_flag)
{
  ctx->context->set_movie_flag(movie_flag);
}

void heif_set_druation_in_TimeScales(const struct heif_image* heifimage, uint64_t duration)
{
  heifimage->image->set_duration_in_Timescales(duration);
}

void heif_set_mvhd_data(struct heif_context* ctx, uint64_t creat_time, uint64_t modified_time, int fps, int duration, int track_count)
{
  ctx->context->set_mvhd_data(creat_time, modified_time, fps, duration, track_count);
}

void heif_set_tkhd_data(struct heif_context* ctx, uint64_t creat_time, uint64_t modified_time, uint32_t track_id, uint64_t duration, uint32_t img_src_width, uint32_t img_src_height)
{
  ctx->context->ctx_set_tkhd_data(creat_time, modified_time, track_id, duration, img_src_width, img_src_height);
}

void heif_set_mdhd_data(struct heif_context* ctx, uint64_t creat_time, uint64_t modified_time, uint32_t fps, uint64_t duration, uint16_t language)
{
  ctx->context->ctx_set_mdhd_data(creat_time, modified_time, fps, duration, language);
}

void heif_set_hvc1_data(struct heif_context* ctx, uint16_t img_src_width, uint16_t img_src_height, uint16_t frame_count)
{
  ctx->context->ctx_set_hvc1_data(img_src_width, img_src_height, frame_count);
}

void heif_calc_stts_data(struct heif_context* ctx)
{
  ctx->context->ctx_calc_stts_data();
}

void heif_set_stss_data(struct heif_context* ctx)
{
  ctx->context->ctx_set_stss_data();
}

void heif_add_movie_box(struct heif_context* ctx)
{
  ctx->context->ctx_add_movie_box();
}
