...
namespace heif {
  ...
  public:
    ...
    void set_max_decoder_threads(int max_threads) {m_max_decoder_threads = max_threads;}
    ...
    Error read_params(libheif_parameters* params);

    int read_grid_params(heif_item_id ID);
    ...
    Error decode_image_planar_for_moov( heif_item_id ID, std::shared_ptr<HeifPixelImage>& img,
                                        heif_colorspace out_colorspace,
                                        heif_chroma  out_chroma,
                                        const struct heif_decoding_options* options,
                                        bool alphaImage) const;
    ...
    void set_movie_flag(bool movie_flag) {m_movie_flag = movie_flag;}
    bool get_movie_flag() const {return m_movie_flag;}
    void set_mvhd_data(uint64_t creation_time, uint64_t modification_time, uint32_t timescale, uint32_t duration, uint32_t next_track_ID);
    void ctx_set_tkhd_data(uint64_t creat_time, uint64_t modified_time, uint32_t track_id, uint64_t duration, uint32_t img_src_width, uint32_t img_src_height);
    void ctx_set_mdhd_data(uint64_t creat_time, uint64_t modified_time, uint32_t timescale, uint64_t duration, uint16_t language);
    void ctx_set_hvc1_data(uint16_t img_src_width, uint16_t img_src_height, uint16_t frame_count);
    void ctx_calc_stts_data();
    void ctx_set_stss_data();
    void ctx_add_movie_box();
  ...
  private:
    ...
    Error interpret_heif_file_for_moov();
