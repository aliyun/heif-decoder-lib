                                         ...
                                         const ColorConversionOptions& options)
{
  ...
  if (chromaSubH == 1 && chromaSubV == 1) {
    ...
    for (long long y = 0; y < (height & ~1); y += 2) {
      const uint8_t* p = &in_p[y * in_stride];

      for (long long x = 0; x < (width & ~1); x += 2) {
    ...
    if (width & 1) {
      ...
      for (long long y = 0; y < height; y += 2) {
        uint8_t r, g, b;
        if (y + 1 < height) {
    ...
    if (height & 1) {
      long long y = height - 1;
      const uint8_t* p = &in_p[y * in_stride];

      for (long long x = 0; x < width; x += 2) {
        uint8_t r, g, b;
        if (x + 1 < width) {
...
template<class Pixel>
class Op_RGBA_GENERAL_to_RGB_GENTRAL : public ColorConversionOperation
{
public:
  std::vector<ColorStateWithCost>
  state_after_conversion(const ColorState& input_state,
                                             ...
                                             const ColorConversionOptions& options)
{
  ...
  int bpp = input_state.bits_per_pixel;
//  if (input_state.nclx_profile) {
//    int matrix = input_state.nclx_profile->get_matrix_coefficients();
//    if (matrix == 0 || matrix == 8 || matrix == 11 || matrix == 14) {
//      return {};
...
//  }
  ...
  output_state.chroma = heif_chroma_interleaved_RGB;
  output_state.has_alpha = false;
  output_state.bits_per_pixel = bpp;
...
#if HAVE_YUV
class Op_YCbCr_to_RGBA_GENERAL : public ColorConversionOperation
{
public:
  std::vector<ColorStateWithCost>
  state_after_conversion(const ColorState& input_state,
...
#if  HAVE_YUV
  ops.push_back(std::make_shared<Op_YCbCr_to_RGBA_GENERAL>());
#endif
  ops.push_back(std::make_shared<Op_RGBA_GENERAL_to_RGB_GENTRAL<uint8_t>>());
  ops.push_back(std::make_shared<Op_RGBA_GENERAL_to_RGB_GENTRAL<uint16_t>>());
