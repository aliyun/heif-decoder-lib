...
enum heif_suberror_code
{
  ...
  heif_suberror_No_mvhd_box = 136,

  heif_suberror_No_trak_box = 137,

  heif_suberror_No_tkhd_box = 138,

  heif_suberror_No_mdia_box = 139,

  heif_suberror_No_mdhd_box = 140,

  heif_suberror_No_minf_box = 141,

  heif_suberror_No_vmhd_box = 142,

  heif_suberror_No_stbl_box = 143,

  heif_suberror_No_stsd_box = 144,

  heif_suberror_No_hvc1_box = 145,

  heif_suberror_No_hvcC_moov_box = 146,

  heif_suberror_No_ccst_box = 147,

  heif_suberror_No_stsz_box = 148,

  heif_suberror_No_stts_box = 149,

  heif_suberror_No_stsc_box = 150,

  heif_suberror_No_stco_box = 151,

  heif_suberror_No_stss_box = 152,
  ...
  heif_suberror_Unsupported_stsc_chunk_unequa1 = 3005,
...
struct heif_decoding_options
{
  ...
  // if ext_dst_enable = true, the buffer ext_dst pointed keep final pixel;
  // if not, ignore ext_dst 
  bool  ext_dst_enable ;
  void* ext_dst ;
  uint32_t ext_dst_len ;
  uint32_t ext_dst_stride;
};

// pass the destined buffer address to the struct,  
LIBHEIF_API 
...
// Android require image with alpha channel which data is premultiplied. Here
// heif_image_premultiply_alpha only work for android RGBA image;
LIBHEIF_API
struct heif_error heif_image_rgba_premultiply_alpha(struct heif_image* img);
...
LIBHEIF_API
void heif_set_movie_flag(struct heif_context*, bool movie_flag);

LIBHEIF_API
void heif_set_druation_in_TimeScales(const struct heif_image* image, uint64_t duration);

LIBHEIF_API
void heif_set_mvhd_data(struct heif_context*, uint64_t creat_time, uint64_t modified_time, int fps, int duration, int track_count);

LIBHEIF_API
void heif_set_tkhd_data(struct heif_context*, uint64_t creat_time, uint64_t modified_time, uint32_t track_id, uint64_t duration, uint32_t img_src_width, uint32_t img_src_height);

LIBHEIF_API
void heif_set_mdhd_data(struct heif_context*, uint64_t creat_time, uint64_t modified_time, uint32_t fps, uint64_t duration, uint16_t language);

LIBHEIF_API
void heif_set_hvc1_data(struct heif_context*, uint16_t img_src_width, uint16_t img_src_height, uint16_t frame_count);

LIBHEIF_API
void heif_calc_stts_data(struct heif_context*);

LIBHEIF_API
void heif_set_stss_data(struct heif_context*);

LIBHEIF_API
void heif_add_movie_box(struct heif_context*);
