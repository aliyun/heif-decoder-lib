...
void init_acceleration_functions_fallback(struct acceleration_functions* accel)
{
  ...
  accel->loop_filter_luma_c_8 = loop_filter_luma<uint8_t>;
  accel->loop_filter_luma_c_16 = loop_filter_luma<uint16_t>;

  accel->loop_filter_chroma_8 = loop_filter_chroma<uint8_t>;
  accel->loop_filter_chroma_16 = loop_filter_chroma<uint16_t>;
  accel->loop_filter_chroma_c_8 = loop_filter_chroma<uint8_t>;
  accel->loop_filter_chroma_c_16 = loop_filter_chroma<uint16_t>;

  accel->sao_band_filter_8 = sao_band_filter_c<uint8_t>;
  accel->sao_band_filter_16 = sao_band_filter_c<uint16_t>;

  accel->sao_edge_filter_8 = sao_edge_filter_c<uint8_t>;
  accel->sao_edge_filter_16 = sao_edge_filter_c<uint16_t>;
}
